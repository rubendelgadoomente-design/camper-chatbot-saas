require('dotenv').config();
const { createClient } = require('@supabase/supabase-js');
const fs = require('fs').promises;
const path = require('path');

const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_KEY);

async function uploadMedia(filePath, category, subcategory, description, companyId = 'generic') {
    console.log(`\n📸 Subiendo archivo: ${filePath}`);
    
    const fileBuffer = await fs.readFile(filePath);
    const ext = path.extname(filePath).toLowerCase();
    const fileName = `${companyId}/${category}/${subcategory}_${Date.now()}${ext}`;
    
    let mimeType = 'application/octet-stream';
    let type = 'image';
    if (ext === '.png') mimeType = 'image/png';
    else if (ext === '.jpg' || ext === '.jpeg') mimeType = 'image/jpeg';
    else if (ext === '.mp4') {
        mimeType = 'video/mp4';
        type = 'video';
    }
    
    // Upload to Supabase Storage
    const { data: storageData, error: storageError } = await supabase.storage
        .from('camperbot-images')
        .upload(fileName, fileBuffer, { contentType: mimeType, upsert: true });
    
    if (storageError) throw new Error(`Storage error: ${storageError.message}`);
    
    // Get public URL
    const { data: urlData } = supabase.storage
        .from('camperbot-images')
        .getPublicUrl(fileName);
    
    const publicUrl = urlData.publicUrl;
    console.log(`   ✅ URL pública: ${publicUrl}`);
    
    // Register in image_library table
    const { error: dbError } = await supabase
        .from('image_library')
        .upsert({ category, subcategory, description, image_url: publicUrl, company_id: companyId }, 
                { onConflict: 'subcategory,company_id' });
    
    if (dbError) throw new Error(`DB error: ${dbError.message}`);
    
    console.log(`   ✅ Registrado en base de datos como [${type}]`);
    return publicUrl;
}

// Main
const [,, filePath, category, subcategory, description, companyId] = process.argv;
if (!filePath || !category || !subcategory || !description) {
    console.log('Uso: node scripts/upload-media.js <archivo> <category> <subcategory> <description> [company_id]');
    process.exit(1);
}

uploadMedia(filePath, category, subcategory, description, companyId || 'generic')
    .then(url => console.log(`\n🎉 Archivo disponible en: ${url}`))
    .catch(err => { console.error('❌ Error:', err.message); process.exit(1); });
